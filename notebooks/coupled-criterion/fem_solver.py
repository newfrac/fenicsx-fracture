...
def fem_solver(mesh, facets, Mechanical_data, Geometrical_data, dl):
    ...
    hor_load_dofs = dolfinx.fem.locate_dofs_topological( V_u, facets.dim, facets.indices[ facets.values == 1 ] )
    ...
    bcs = [dolfinx.fem.dirichletbc( dolfinx.fem.Constant(mesh, 0.), hor_sym_dofs, V_u.sub(1) ), 
          dolfinx.fem.dirichletbc( dolfinx.fem.Constant( mesh, petsc4py.PETSc.ScalarType((0,1)) ), hor_load_dofs, V_u )]
    ...
    functional   = ufl.inner(sigma(u),eps(u)) * ufl.dx - ufl.dot(f,u) * ufl.dx - ufl.dot(q,u) * ufl.ds
    ...
    snes = petsc4py.PETSc.PETSc.SNES().create()
    ...
    if dl==0:
        with dolfinx.io.XDMFFile(MPI.COMM_WORLD, "output/u_undamaged.xdmf", "w") as file:
    ...
    energy = dolfinx.fem.assemble_scalar(dolfinx.fem.form( functional )) # calculation in 1/2 of the whole domain
    ...
    if dl == 0:  
        ...
        sig_expres = dolfinx.fem.Expression(sigma(u), V_s.element.interpolation_points())
        ...
        colliding_cells = dolfinx.geometry.compute_colliding_cells(mesh, cell_candidates, points_eval.T)
        ...
        return energy, force, tensile_sig[0:len(tensile_sig)-2], vector_dl[0:len(tensile_sig)-2] 
